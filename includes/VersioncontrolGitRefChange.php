...
abstract class VersioncontrolGitRefChange {
  ...
  */
   ...
   *
   * Note that, in the case of a non-fast-forward push, this does NOT contain
   * the range of excised commits - only newly-introduced ones.
   *
  ...
  public function getNewCommit() {
    if (!$this->eventDeletedMe()) {
      $commits = $this->repository->loadCommits(array(), array('revision' => $this->new_sha1));
  ...
  */
  public function getOldCommit() {
    if (!$this->eventCreatedMe()) {
      $commits = $this->repository->loadCommits(array(), array('revision' => $this->old_sha1));
      return reset($commits);
    }
  }
  ...
  public function eventDeletedMe() {
    return $this->new_sha1 == GIT_NULL_REV;
  }
  ...
  public function eventCreatedMe() {
    return $this->old_sha1 == GIT_NULL_REV;
  }
